...
## Evaluations of pro- and anti-US authors
Overall, participants gave higher ratings to the pro-US author (*M* = `r merged$proauth_avg %>% mean(na.rm = T) %>% formatC(3)`, *SD* = `r merged$proauth_avg %>% sd(na.rm = T) %>% formatC(3)` than the anti-US author (*M* = `r merged$antiauth_avg %>% mean(na.rm = T) %>% formatC(3)`, *SD* = `r merged$antiauth_avg %>% mean(na.rm = T) %>% formatC(3)`). This difference was statistically significant, `r apa_print(author_liking)$statistic`.
