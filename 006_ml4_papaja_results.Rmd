...
## Participants
Twenty-one data collection efforts from 18 unique sites collected data from a total of 2281 participants.
...
Some In House sites collected data before the trial's registration; that data was discarded from this confirmatory test, in line with the replication. This excluded a further 545 participants. 
...
*Exclusion Set 2:* All prior exclusions, and further exclude participants who did not identify as White or who indicated they were born outside the United States. This reduces the *N* to `r formatC(nrow(merged_excl_2))`. This sample size gives us 95% power to detect a condition effect of *d* = `r pwr::pwr.t.test(n = nrow(merged_excl_2)/2, power = .95, type = "two.sample", alternative = "two.sided")$d %>% round(2)`.  
...
There may have been a mortality salience effect at some sites and not others, so we next examined how much variation was observed among effect sizes (e.g., heterogeneity). This sort of variation did not exceed variation expected by chance (e.g., sampling variance) regardless of exclusion rule: Exclusion Set 1: *Q*(`r summary(random1)$Q.stat$Q.df`) = `r summary(random1)$Q.stat$Q`, *p* = `r summary(random1)$Q.stat$pval`; Exclusion Set 2, *Q*(`r summary(random2)$Q.stat$Q.df`) = `r summary(random2)$Q.stat$Q`, *p* = `r summary(random2)$Q.stat$pval`; Exclusion Set 3: *Q*(`r summary(random3)$Q.stat$Q.df`) = `r summary(random3)$Q.stat$Q`, *p* = `r summary(random3)$Q.stat$pval`. 
...
Again, significant heterogeneity was not observed: Exclusion Set 1 *Q*(`r summary(mixed1)$Q.stat$Q.df`) = `r summary(mixed1)$Q.stat$Q`, *p* = `r summary(mixed1)$Q.stat$pval`; Exclusion Set 2, *Q*(`r summary(mixed2)$Q.stat$Q.df`) = `r summary(mixed2)$Q.stat$Q`, *p* = `r summary(mixed2)$Q.stat$pval`; Exclusion Set 3, *Q*(`r summary(mixed3)$Q.stat$Q.df`) = `r summary(mixed3)$Q.stat$Q`, *p* = `r summary(mixed3)$Q.stat$pval`.
