...
Is the anticipated effect stronger among participants who preferred the pro-US author? When restricting the analysis to only participants at Author Advised sites who preferred the pro-US author and using only the first exclusion rule, the mortality salience and control groups significantly differed in their levels of preference for the pro-US author over the anti-US author: `r report_meta(random1_proUSonly, "Intercept1")`. However, this test was not significant when using the second or third exclusion rule:
Exclusion Set 2: `r report_meta(random2_proUSonly, "Intercept1")`; 
Exclusion Set 3: `r report_meta(random3_proUSonly, "Intercept1")`. 
**[Note: the above analyses do not converge (OpenMX status: 5), perhaps because the heterogeneity is estimated as very small, nearly zero. metafor::rma() does not have such an error. If I run a fixed-effects model in metaSEM then I get the same results across metaSEM::meta and metafor::rma. The SEs and some p values' significances is sensitive to this!]**
