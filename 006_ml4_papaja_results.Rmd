...
# Work in progress
Power calculations in this document are hardcoded, not calculated on the fly using RMarkdown, and can be inaccurate. 
...
# TODO: strip out the redundant code.
#   Instead of rerunning the models, run them once in 002_ml4analysis.R
#   And then load and call them in this RMD
...
There may have been a mortality salience effect at some sites and not others, so we next examined how much variation was observed among effect sizes (e.g., heterogeneity). This sort of variation did not exceed variation expected by chance (e.g., sampling variance) regardless of exclusion rule: Exclusion Set 1: *Q*(`r random1$Q.stat$Q.df`) = `r random1$Q.stat$Q`, *p* = `r random1$Q.stat$pval`; Exclusion Set 2, *Q*(`r random2$Q.stat$Q.df`) = `r random2$Q.stat$Q`, *p* = `r random2$Q.stat$pval`; Exclusion Set 3: *Q*(`r random3$Q.stat$Q.df`) = `r random3$Q.stat$Q`, *p* = `r random3$Q.stat$pval`. 
...
Again, significant heterogeneity was not observed: Exclusion Set 1 *Q*(`r mixed1$Q.stat$Q.df`) = `r mixed1$Q.stat$Q`, *p* = `r mixed1$Q.stat$pval`; Exclusion Set 2, *Q*(`r mixed2$Q.stat$Q.df`) = `r mixed2$Q.stat$Q`, *p* = `r mixed2$Q.stat$pval`; Exclusion Set 3, *Q*(`r mixed3$Q.stat$Q.df`) = `r mixed3$Q.stat$Q`, *p* = `r mixed3$Q.stat$pval`.
